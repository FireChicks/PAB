...
function moveToCpuInfo(partName) {
    axios.get('/cpu/getInfo', {
        params: {
            name: partName
        }
    ...
    .then(function(response) {
        ...
        .then(function(response) {
            ...
            var cpuName = document.createElement('a');
            cpuNameCell.colSpan = '2';
            cpuName.textContent = cpu.cpuName;
            cpuName.style.fontWeight = 'bold';
            cpuParamName = cpu.cpuName;
            cpuNameCell.appendChild(cpuName);
            ...
            Object.keys(cpu).forEach(function(key) {
                if (key !== 'cpuName' && key !== 'amazon_Link' && key !== 'amazon_img_link') {
                    ...
                    infoLabelCell.style.fontWeight = 'bold';
            ...
            // CPU 정보 페이지에서 "추가" 버튼을 클릭했을 때의 동작을 하드코딩.
                var addButton = document.createElement('button');
                addButton.textContent = '추가';
                addButton.className = 'btn btn-primary';
                addButton.addEventListener('click', function() {
                    var infoName = 'cpuName';
                    var info = encodeURIComponent(cpuParamName);
                    var url = '/modal/addParts?infoName=' + infoName + '&info=' + info;
                    location.href = url;
                });
...
function moveToMbInfo(partName) {
  axios
    .get('/mb/getInfo', {
      params: {
        name: partName
    ...
    .then(function(response) {
      ...
      axios
        ...
        .then(function(response) {
          ...
          var mbName = document.createElement('a');
          mbNameCell.colSpan = '2';
          mbName.textContent = mb.mbName;
          mbName.style.fontWeight = 'bold';
          mbParamName = mb.mbName;
          mbNameCell.appendChild(mbName);
          ...
          // 메인보드 정보 페이지에서 "추가" 버튼을 클릭했을 때의 동작을 하드코딩.
          var addButton = document.createElement('button');
          addButton.textContent = '추가';
          addButton.className = 'btn btn-primary';
          addButton.addEventListener('click', function() {
            var infoName = 'mainBoard';
            var info = encodeURIComponent(mbParamName);
            var url = '/modal/addParts?infoName=' + infoName + '&info=' + info;
            location.href = url;
          });
          var buttonContainer = document.createElement('div');
...
function moveToRamInfo(partName) {
    axios.get('/ram/getInfo', {
        params: {
            name: partName
        }
    ...
    .then(function(response) {
        var ramID = response.data.ramId;
        var ramParamName;

        // 추출한 RAM ID 값을 활용하여 필요한 작업을 수행
        if (ramID !== null) {
            console.log('RAM ID:', ramID);
            // ...
        } else {
            console.log('RAM ID를 찾을 수 없습니다.');
        }
        ...
        .then(function(response) {
            ...
            // 클릭 가능한 링크가 있는 RAM 이름 행 생성
            var ramNameRow = document.createElement('tr');
            var ramNameCell = document.createElement('td');
            var ramName = document.createElement('a');
            ramNameCell.colSpan = '2';
            ramName.textContent = ram.ramName;
            ramName.style.fontWeight = 'bold';
            ramParamName = ram.ramName;
            ramNameCell.appendChild(ramName);
            ramNameRow.appendChild(ramNameCell);
            tbody.appendChild(ramNameRow);

            // RAM 정보 행 생성
            Object.keys(ram).forEach(function(key) {
                if (key !== 'ramName' && key !== 'amazon_Link' && key !== 'amazon_img_link') {
                    var infoRow = document.createElement('tr');
                    var infoLabelCell = document.createElement('td');
                    var infoValueCell = document.createElement('td');
                    infoLabelCell.textContent = key;
            ...
            // RAM 정보 페이지에서 "추가" 버튼을 클릭했을 때의 동작을 하드코딩.
            var addButton = document.createElement('button');
            addButton.textContent = '추가';
            addButton.className = 'btn btn-primary';
            addButton.addEventListener('click', function() {
                var infoName = 'ram';
                var info = encodeURIComponent(ramParamName);
                var url = '/modal/addParts?infoName=' + infoName + '&info=' + info;
        ...
        .catch(function(error) {
            console.error('RAM 정보를 불러오는 중 에러가 발생했습니다:', error);
        });
    })
    .catch(function(error) {
        console.error('RAM ID를 불러오는 중 에러가 발생했습니다:', error);
    });
}
...
function moveToStoInfo(partName) {
    axios.get('/sto/getInfo', {
        params: {
            name: partName
        }
    ...
    .then(function(response) {
        var stoID = response.data.stoID;
        var stoParamName;

        // 추출한 Storage ID 값을 활용하여 필요한 작업을 수행
        if (stoID !== null) {
            console.log('Storage ID:', stoID);
            // ...
        } else {
            console.log('Storage ID를 찾을 수 없습니다.');
        }
        ...
        .then(function(response) {
            ...
            // 클릭 가능한 링크가 있는 Storage 이름 행 생성
            var stoNameRow = document.createElement('tr');
            var stoNameCell = document.createElement('td');
            var stoName = document.createElement('a');
            stoNameCell.colSpan = '2';
            stoName.textContent = storage.stoName;
            stoName.style.fontWeight = 'bold';
            stoParamName = storage.stoName;
            stoNameCell.appendChild(stoName);
            ...
            // Storage 정보 페이지에서 "추가" 버튼을 클릭했을 때의 동작을 하드코딩.
            var addButton = document.createElement('button');
            addButton.textContent = '추가';
            addButton.className = 'btn btn-primary';
            addButton.addEventListener('click', function() {
                var infoName = 'storage';
                var info = encodeURIComponent(stoParamName);
                var url = '/modal/addParts?infoName=' + infoName + '&info=' + info;
                window.location.href = url;
            });
            container.appendChild(addButton);
        ...
        .catch(function(error) {
            console.error('Storage 정보를 불러오는 중 에러가 발생했습니다:', error);
        });
    })
    .catch(function(error) {
        console.error('Storage ID를 불러오는 중 에러가 발생했습니다:', error);
    });
}
...
function moveToPowInfo(partName) {
    axios.get('/pow/getInfo', {
        params: {
            name: partName
        }
    ...
    .then(function(response) {
        var powID = response.data.powId;
        var powParamName;

        // 추출한 Power ID 값을 활용하여 필요한 작업을 수행
        if (powID !== null) {
            console.log('Power ID:', powID);
            // ...
        } else {
            console.log('Power ID를 찾을 수 없습니다.');
        }
        ...
        .then(function(response) {
            ...
            var imgCell = document.createElement('td');
            var imgElement = document.createElement('img');
            imgElement.src = power.amazon_img_link;
            imgElement.style.width = '200px';
            imgElement.style.height = '200px';
            imgCell.colSpan = '2';
            imgCell.appendChild(imgElement);
            ...
            // 클릭 가능한 링크가 있는 Power 이름 행 생성
            var powNameRow = document.createElement('tr');
            var powNameCell = document.createElement('td');
            var powName = document.createElement('a');
            powNameCell.colSpan = '2';
            powName.textContent = power.powName;
            powName.style.fontWeight = 'bold';
            powParamName = power.powName;
            powNameCell.appendChild(powName);
            powNameRow.appendChild(powNameCell);
            tbody.appendChild(powNameRow);

            // Power 정보 행 생성
            Object.keys(power).forEach(function(key) {
                if (key !== 'powName' && key !== 'amazonLink' && key !== 'amazon_img_link') {
                    ...
                    infoLabelCell.textContent = key;
                    infoLabelCell.style.fontWeight = 'bold';
                    infoValueCell.textContent = power[key];
                    infoRow.appendChild(infoLabelCell);
                    infoRow.appendChild(infoValueCell);
                    tbody.appendChild(infoRow);
                }
            ...
            // Power 정보 페이지에서 "추가" 버튼을 클릭했을 때의 동작을 하드코딩.
            var addButton = document.createElement('button');
            addButton.textContent = '추가';
            addButton.className = 'btn btn-primary';
            addButton.addEventListener('click', function() {
                var infoName = 'power';
                var info = encodeURIComponent(powParamName);
                var url = '/modal/addParts?infoName=' + infoName + '&info=' + info;
                window.location.href = url;
            });
            container.appendChild(addButton);
        ...
        .catch(function(error) {
            console.error('Power 정보를 불러오는 중 에러가 발생했습니다:', error);
        });
    })
    .catch(function(error) {
        console.error('Power ID를 불러오는 중 에러가 발생했습니다:', error);
    });
}
...
function moveToGpuInfo(partName) {
    axios.get('/gpu/getInfo', {
        params: {
            name: partName
        }
    ...
    .then(function(response) {
        ...
        .then(function(response) {
            ...
            var gpuName = document.createElement('a');
            gpuNameCell.colSpan = '2';
            gpuName.textContent = gpu.gpuName;
            gpuName.style.fontWeight = 'bold';
            gpuParamName = gpu.gpuName;
            gpuNameCell.appendChild(gpuName);
            ...
            // GPU 정보 페이지에서 "추가" 버튼을 클릭했을 때의 동작을 하드코딩.
            var addButton = document.createElement('button');
            addButton.textContent = '추가';
            addButton.className = 'btn btn-primary';
            addButton.addEventListener('click', function() {
                var infoName = 'gpu';
                var info = encodeURIComponent(gpuParamName);
                var url = '/modal/addParts?infoName=' + infoName + '&info=' + info;
                window.location.href = url;
            });
            container.appendChild(addButton);
        ...
        .catch(function(error) {
            console.error('GPU 정보를 불러오는 중 에러가 발생했습니다:', error);
        });
    })
    .catch(function(error) {
        console.error('GPU ID를 불러오는 중 에러가 발생했습니다:', error);
    });
}
