...
async function inputToTableBody(row, tableBody, info) {
  if (info.ramName.length <= MAX_RAMNAME_LENGTH) {
    row.insertCell(0).innerHTML = `<a href="${info.amazon_Link}">${info.ramName}</a>`;
  } else {
    row.insertCell(0).innerHTML = `<a href="${info.amazon_Link}">${info.ramName.substring(0, MAX_RAMNAME_LENGTH) + '...'}</a>`;
  }
  ...
  row.insertCell(7).innerHTML = '<a class="btn btn-primary" href="/estimate?infoName=ram&info='+ info.ramName.substring(0, 50) + '...' + '&originalPage=' + url +'">추가</a>';
